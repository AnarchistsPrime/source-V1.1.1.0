...
uint256 hashGenesisBlock("0x000000008afd70438709390e1a1b4e64c81437ffb244d785b7d6029d7b1ac95e");
...
const string strMessageMagic = "AnarchistsPrime Signed Message:\n";
...
static const int64 nTargetTimespan = 28800; // 8 hours
static const int64 nTargetSpacing = 180; //3 min
static const int64 nInterval = nTargetTimespan / nTargetSpacing; 

static const int64 nBlockDigishieldStart = 13000;
static const int64 nTargetTimespanDigishield = 180; // 3 min
static const int64 nTargetSpacingDigishield = 180; //3 min
static const int64 nIntervalDigishield = nTargetTimespanDigishield / nTargetSpacingDigishield; 
//
...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime)
{
    ...
    {
		if (nBestHeight+1<nBlockDigishieldStart) {  
        // Maximum 400% adjustment...
        bnResult *= 4;
        // ... in best-case exactly 4-times-normal target time
        nTime -= nTargetTimespan*4;
		} 
		else // start digisheild
		{ 
		 // Maximum 15% adjustment...
		 bnResult = (bnResult * 115) / 100; 
		 // ... in best-case exactly 4-times-normal target time 
		 nTime -= nTargetTimespanDigishield*4; 
		}
    }
...
unsigned int DigiShield(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();
       ...
       int64 retargetTimespan = nTargetTimespanDigishield;
       int64 retargetSpacing = nTargetSpacingDigishield;
       int64 retargetInterval = nIntervalDigishield;
...
{		
      if (fTestNet) { return GetNextWorkRequired_V1(pindexLast, pblock); }
      if (pindexLast->nHeight+1 < nBlockDigishieldStart) { return GetNextWorkRequired_V1(pindexLast, pblock); }
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x000000008afd70438709390e1a1b4e64c81437ffb244d785b7d6029d7b1ac95e");
    }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AnarchistsPrimeMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AnarchistsPrimeMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("AnarchistsPrimeMiner started\n");
    ...
    try { while (true) {
        ...
        printf("Running AnarchistsPrimeMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AnarchistsPrimeMiner terminated\n");
        throw;
    }
}
